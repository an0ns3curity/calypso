...
class SplitChallengeBlock(pypuf.simulation.base.Simulation):
    ...
    def __init__(self, n, seed, m, noisiness=0, external_weights_list=None, external_bias_list=None):
        assert n % m == 0, f'n must be multiple of m={m}, got {n}'
        assert n > 0, f'n must be positive, got {n}'
        self.n = n
        self.o = n // m
        if(external_weights_list is None and  external_bias_list is None):
            self.pufs = [
            ...
            self.pufs = [
                pypuf.simulation.ArbiterPUF(
                    ...
                    seed=self.seed(f'{self.__class__} seed={seed} {i}'),
                    transform=pypuf.simulation.ArbiterPUF.transform_id,
                    noisiness=noisiness, external_weights=external_weights_list[i], external_biases=external_bias_list[i]
                )
                    for i in range(m)
            ]
...
class TBlock(pypuf.simulation.base.Simulation):
    ...
    def __init__(self, n1, n2, seed, parity_vector=None):
        """
        n1 "public" bits
        n2 "secret" bits
        """
        ...
        if(parity_vector is None):
            self.parities_of = [
                rng.choice(a=n2, size=max(n2 // 2, 1), replace=False)
                for _ in range(n1)
            ]
        else:
            self.parities_of = parity_vector
...
class LPPUFv1(pypuf.simulation.base.Simulation):
    ...
    def __init__(self, n, seed, m, noisiness_1=0, noisiness_2=0,
                 split_challenge_block_weight_list=None,
                 split_challenge_block_bias_list=None, parity_vector=None,
                 xor_arbiter_weight_vector = None,
                 xor_arbiter_bias_vector = None):
        super().__init__()
        self.layer = [
            SplitChallengeBlock(n=n, m=m, noisiness=noisiness_1, seed=self.seed(f'LPPUFv1 {seed} Layer 1'), external_weights_list=split_challenge_block_weight_list, external_bias_list=split_challenge_block_bias_list),
            TBlock(n1=n, n2=m, seed=self.seed(f'LPPUFv1 {seed} Layer 2'), parity_vector=parity_vector),
            pypuf.simulation.XORArbiterPUF(n=n, k=m, noisiness=noisiness_2, seed=self.seed(f'LPPUFv1 {seed} Layer 3'), external_weights = xor_arbiter_weight_vector, external_biases=xor_arbiter_bias_vector),
        ]
