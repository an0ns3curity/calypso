...
class XORArbiterPUF(NoisyLTFArray):
    ...
    def __init__(self, n: int, k: int, seed: int = None, transform: Union[Callable, str] = None,
                 noisiness: float = 0, external_weights=None, external_biases=None) -> None:
        if seed is None:
            seed = 'default'
        ...
        input_bias_array = None
        if(external_weights is None): 
            input_weight_array = np.concatenate((
                weight_rng.normal(loc=0, scale=.5, size=(k, 1)),
                weight_rng.normal(loc=0, scale=.5, size=(k, n - 1)),
            ), axis=1)
        else:
            input_weight_array = external_weights

        if(external_biases is None):  
            input_bias_array = weight_rng.normal(loc=0, scale=.5, size=(k,))
        else:
            input_bias_array = external_biases

        super().__init__(
            weight_array=input_weight_array,
            bias=input_bias_array,
...
class ArbiterPUF(XORArbiterPUF):
    ...
    def __init__(self, n: int, seed: int = None, transform: Union[Callable, str] = None, noisiness: float = 0, external_weights=None, external_biases=None) -> None:
        super().__init__(n, 1, seed, transform, noisiness, external_weights, external_biases)
...
class InterposePUF(Simulation):
    ...
    def __init__(self, n: int, k_down: int, k_up: int = 1, interpose_pos: int = None, seed: int = None,
                 noisiness: float = 0,
                 external_weights_up=None,
                 external_bias_up=None,
                 external_weights_down=None,
                 external_bias_down=None) -> None:
        super().__init__()
        if seed is None:
            seed = 'default'
        seed_up = self.seed(f'interpose puf {seed} up')
        seed_down = self.seed(f'interpose puf {seed} down')
        self.up = XORArbiterPUF(n, k_up, seed_up, XORArbiterPUF.transform_atf, noisiness, external_weights_up, external_bias_up)
        self.down = XORArbiterPUF(n + 1, k_down, seed_down, XORArbiterPUF.transform_atf, noisiness, external_weights_down, external_bias_down)
